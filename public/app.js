...
async function loadLessons() {
    try {
        ...
        if (lessons.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: #999; padding: 40px;">Žádné lekce k zobrazení. Přidejte první lekci!</p>';
            return;
        }
    ...
    try {
        ...
        if (response.ok) {
            if (result.status === 'confirmed') {
                ...
                showNotification('Lekce je plná. Účastník byl přidán na čekací listinu.', 'info');
            }
            form.reset();
            loadLessons();
        } else {
...
async function loadSubstitutionLessons() {
    ...
    try {
        const response = await fetch(`${API_URL}/substitutions/${encodeURIComponent(ageGroup)}`);
        const lessons = await response.json();

        if (lessons.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: #999; padding: 20px;">Žádné dostupné náhradní lekce pro tuto věkovou skupinu.</p>';
            return;
        }

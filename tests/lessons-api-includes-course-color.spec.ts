...
test.describe.serial("GET /api/lessons includes courseColor and courseName", () => {
	...
	test("lesson with courseId includes courseColor and courseName from the linked course", async () => {
		...
		const lesson = lessons.find((l: { id: string }) => l.id === "lesson_color_test");
	...
	test("lesson without courseId has courseColor and courseName as null", async () => {
		...
		const lesson = lessons.find((l: { id: string }) => l.id === "lesson_no_course");
		expect(lesson).toBeDefined();

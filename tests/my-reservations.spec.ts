...
test.describe
	.serial("My reservations — participant scope", () => {
		let p1Id: string;
		let p2Id: string;
		let cookie1: string;
		let cookie2: string;
		...
		test("participant sees only their own registrations", async () => {
			const res = await fetch(
				`${BASE}/api/participants/${p1Id}/registrations`,
				{
		...
		test("participant can cancel their own future registration (before midnight)", async () => {
			const res = await fetch(
				`${BASE}/api/participants/${p1Id}/cancel-registration`,
				{
					method: "POST",
		...
		test("participant cannot cancel another participant's registration", async () => {
			const res = await fetch(
				`${BASE}/api/participants/${p2Id}/cancel-registration`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json", Cookie: cookie1 },
					body: JSON.stringify({ registrationId: "reg_alice_1" }),

...
test.describe.serial("Customer flow", () => {
	...
	test("2. after cancel, mom sees sister skupinka lessons as substitution candidates", async () => {
		await registrationManager.participantCancelRegistration("reg_own_1", momId);
		...
		const ownCourseIds = new Set(participantCourses.map((c) => c.id as string));
	...
	test("5. mom cannot self-register for substitution without credit", async () => {
		...
		expect(result.success).toBe(false);
		const resultWithFlag = result as { success: false; error: string; noCredit?: boolean };
